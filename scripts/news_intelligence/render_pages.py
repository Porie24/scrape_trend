#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""HTML renderers for the news-intelligence word cloud + ranked list.
Shared design tokens with the rest of the Isan Household Financial Fragility
Toolkit artifact family (warm cream / dark theme)."""
...
def render_wordcloud(phrase_data, n_headlines, out_path, classify_fn):
    ...
    pie_data = [{"cat": c, "label": PIE_LABEL[c], "value": v}
                for c, v in sorted(pie_sums.items(), key=lambda kv: -kv[1])]
    pie_json = json.dumps(pie_data, ensure_ascii=False)

    html = HTML_WORDCLOUD_TEMPLATE.replace("__BASE_VARS__", BASE_CSS_VARS) \
        .replace("__DARK_VARS__", DARK_CSS_VARS) \
        .replace("__N_HEADLINES__", str(n_headlines)) \
        .replace("__WORDS_JSON__", words_json) \
        .replace("__PIE_JSON__", pie_json)
    out_path.write_text(html, encoding="utf-8")


def render_wordlist(phrase_data, n_headlines, out_path, classify_fn):
    words = phrase_data["words"]
    groups = {}
    for w in words:
        cat = classify_fn(w["text"])
    ...
    for cat in groups:
        groups[cat].sort(key=lambda x: -x["count"])

    groups_json = json.dumps([
        {"cat": cat, "label": CAT_LABEL_FULL[cat], "words": groups[cat]}
        for cat in CAT_ORDER if cat in groups
    ], ensure_ascii=False)

    html = HTML_WORDLIST_TEMPLATE.replace("__BASE_VARS__", BASE_CSS_VARS) \
        .replace("__DARK_VARS__", DARK_CSS_VARS) \
        .replace("__N_HEADLINES__", str(n_headlines)) \
...
<style>
  :root { __BASE_VARS__ }
  @media (prefers-color-scheme: dark) { :root:not([data-theme="light"]) { __DARK_VARS__ } }
  :root[data-theme="dark"] { __DARK_VARS__ }
  * { box-sizing: border-box; }
  html, body { background: var(--bg); }
  body { color: var(--ink); font-family: var(--font-th); margin: 0; padding: 48px 20px 90px; -webkit-font-smoothing: antialiased; line-height: 1.7; }
  .wrap { max-width: 920px; margin: 0 auto; }
  .eyebrow { font-size: 12px; letter-spacing: 0.14em; text-transform: uppercase; color: var(--c1); font-weight: 700; margin: 0 0 12px; }
  h1 { font-family: var(--font-serif-th); font-size: clamp(24px, 4vw, 32px); line-height: 1.3; margin: 0 0 12px; text-wrap: balance; font-weight: 600; }
  .dek { font-size: 15px; line-height: 1.7; color: var(--ink-muted); margin: 0 0 8px; max-width: 72ch; }
  .source-note { font-size: 12.5px; color: var(--ink-faint); margin: 0 0 22px; }
  .card { background: var(--surface); border: 1px solid var(--rule); border-radius: 10px; padding: 20px; }
  #cloud { display: block; width: 100%; height: auto; }
  .legend { display: flex; gap: 18px; margin-top: 16px; font-size: 12px; color: var(--ink-muted); flex-wrap: wrap; }
  .legend-item { display: flex; align-items: center; gap: 7px; }
  .legend-dot { width: 9px; height: 9px; border-radius: 50%; flex: none; }
  .callout { background: var(--surface); border: 1px solid var(--rule); border-left: 3px solid var(--c1); border-radius: 8px; padding: 16px 18px; margin: 20px 0 0; font-size: 13px; line-height: 1.75; color: var(--ink-muted); }
  .callout strong { color: var(--ink); }
  .section-divider { font-family: var(--font-serif-th); font-size: 15px; font-weight: 700; margin: 36px 0 6px; }
  .pie-wrap { display: flex; gap: 28px; align-items: center; flex-wrap: wrap; }
  #pie { flex: none; }
  .pie-legend { display: flex; flex-direction: column; gap: 10px; font-size: 13px; color: var(--ink-muted); flex: 1; min-width: 220px; }
  .pie-legend-item { display: flex; align-items: baseline; gap: 8px; }
...
<div class="wrap">
  ...
  <footer>สร้างโดยอัตโนมัติ · ตัดคำภาษาไทยด้วย pythainlp · เป็นภาพประกอบเชิงคุณภาพ ไม่ใช่ผลการวิเคราะห์เชิงสถิติ</footer>
</div>
<script>
(function () {
  var WORDS = __WORDS_JSON__;
  var PIE = __PIE_JSON__;
  ...
  var CAT_KEYWORDS = [
    ...
    ['c8', ['สภาพคล่อง', 'เงินสำรอง', 'เงินสด', 'ไม่มีเงิน', 'ทรัพย์สิน', 'เก็บออม', 'ออมเงิน']],
    ['c9', ['ตกงาน', 'ว่างงาน', 'เกษตรกร', 'รับจ้าง', 'อาชีพอิสระ', 'รายได้ไม่มั่นคง', 'รายได้น้อย']],
    ['c10', ['เครดิตบูโร', 'แบล็คลิสต์', 'ประวัติ', 'ฐานข้อมูล']],
    ...
    ['c2', ['แก้', 'พักหนี้', 'ลดหนี้', 'ปรับโครงสร้าง', 'เจรจา', 'ไกล่เกลี่ย', 'มหกรรม', 'อุ้ม', 'เยียวยา', 'รัดเข็มขัด', 'รีไฟแนนซ์', 'รวมหนี้']],
  ];
  function classify(text) {
    for (var i = 0; i < CAT_KEYWORDS.length; i++) {
      var cat = CAT_KEYWORDS[i][0], keywords = CAT_KEYWORDS[i][1];
      for (var j = 0; j < keywords.length; j++) if (text.indexOf(keywords[j]) !== -1) return cat;
    }
    if (text === 'หนี้' || text === 'ครัวเรือน') return 'c1';
    return 'c6';
  }
  function colorFor(word) { return getColor('--' + classify(word)); }

  var canvas = document.getElementById('cloud');
  var ctx = canvas.getContext('2d');
  var W = canvas.width, H = canvas.height;
  var counts = WORDS.map(function(w) { return w.count; });
  var maxC = Math.max.apply(null, counts), minC = Math.min.apply(null, counts);
  function fontSizeFor(c) { var t = (Math.log(c) - Math.log(minC)) / (Math.log(maxC) - Math.log(minC)); return 13 + t * 46; }
  ...
  try { var bf = getComputedStyle(document.body).fontFamily; if (bf) fontFamily = bf; } catch (e) {}
  var cx = W / 2, cy = H / 2;

  WORDS.forEach(function(w) {
    var size = fontSizeFor(w.count);
    ctx.font = '700 ' + size + 'px ' + fontFamily;
    var metrics = ctx.measureText(w.text);
    var tw = metrics.width;
    if (!isFinite(tw) || tw <= 0) tw = w.text.length * size * 0.62;
    var th = size * 1.05;
    var angle = 0, radius = 0, found = false;
    var box = { x: cx - tw / 2, y: cy - th / 2, w: tw, h: th };
    for (var attempt = 0; attempt < 2000; attempt++) {
      var x = cx + radius * Math.cos(angle) - tw / 2;
      var y = cy + radius * Math.sin(angle) * 0.62 - th / 2;
      var candidate = { x: x, y: y, w: tw, h: th };
      var inBounds = x > 4 && y > 4 && x + tw < W - 4 && y + th < H - 4;
      if (inBounds && !overlaps(candidate)) { box = candidate; found = true; break; }
      if (inBounds) box = candidate;
      angle += 0.35; radius += 1.6;
    }
    placed.push(box);
    ctx.fillStyle = colorFor(w.text);
    ctx.textBaseline = 'top';
    ctx.fillText(w.text, box.x, box.y);
  });

  var total = PIE.reduce(function (s, d) { return s + d.value; }, 0);
  ...
  var pcx = 130, pcy = 130, pr = 110, pangle = -Math.PI / 2;
  PIE.forEach(function (d) {
    var frac = d.value / total, a0 = pangle, a1 = pangle + frac * Math.PI * 2;
    var x0 = pcx + pr * Math.cos(a0), y0 = pcy + pr * Math.sin(a0);
    var x1 = pcx + pr * Math.cos(a1), y1 = pcy + pr * Math.sin(a1);
    var largeArc = (a1 - a0) > Math.PI ? 1 : 0;
    var path = 'M' + pcx + ',' + pcy + ' L' + x0.toFixed(2) + ',' + y0.toFixed(2) + ' A' + pr + ',' + pr + ' 0 ' + largeArc + ' 1 ' + x1.toFixed(2) + ',' + y1.toFixed(2) + ' Z';
    pieEl.appendChild(el('path', { d: path, class: 'pie-slice-' + d.cat }));
    pangle = a1;
  });
  var legend = document.getElementById('pie-legend');
  PIE.forEach(function (d) {
    var pct = Math.round((d.value / total) * 100);
    var row = document.createElement('div'); row.className = 'pie-legend-item';
    var dot = document.createElement('span'); dot.className = 'pie-legend-dot pie-dot-' + d.cat; row.appendChild(dot);
    var label = document.createElement('span'); label.className = 'pie-legend-label'; label.textContent = d.label; row.appendChild(label);
    var pctEl = document.createElement('span'); pctEl.className = 'pie-legend-pct'; pctEl.textContent = pct + '% (' + d.value + ')'; row.appendChild(pctEl);
...
HTML_WORDLIST_TEMPLATE = r'''<meta charset="utf-8">
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .eyebrow { font-size: 12px; letter-spacing: 0.14em; text-transform: uppercase; color: var(--c-core); font-weight: 700; margin: 0 0 12px; }
  h1 { font-family: var(--font-serif-th); font-size: clamp(24px, 4vw, 32px); line-height: 1.3; margin: 0 0 12px; text-wrap: balance; font-weight: 600; }
  .dek { font-size: 15px; line-height: 1.7; color: var(--ink-muted); margin: 0 0 8px; max-width: 68ch; }
  .source-note { font-size: 12.5px; color: var(--ink-faint); margin: 0 0 32px; }
  .group { margin-bottom: 30px; }
  .group-head { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; }
  .group-dot { width: 10px; height: 10px; border-radius: 50%; flex: none; }
  .group-title { font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; color: var(--ink); }
  .group-count { font-size: 12px; color: var(--ink-faint); }
  .row { display: grid; grid-template-columns: 190px 1fr 34px; align-items: center; gap: 12px; padding: 5px 0; }
  .row-word { font-size: 14.5px; font-weight: 700; color: var(--ink); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .row-track { height: 10px; background: var(--track); border-radius: 5px; overflow: hidden; }
  .row-bar { height: 100%; border-radius: 5px; }
  .row-count { font-size: 12.5px; color: var(--ink-muted); text-align: right; font-variant-numeric: tabular-nums; }
  .callout { background: var(--surface); border: 1px solid var(--rule); border-left: 3px solid var(--c-core); border-radius: 8px; padding: 16px 18px; margin: 8px 0 0; font-size: 13px; line-height: 1.75; color: var(--ink-muted); }
  .callout strong { color: var(--ink); }
  a.forward { display: inline-block; margin: 18px 0 0; font-size: 13px; color: var(--ink-faint); text-decoration: none; border-bottom: 1px dotted var(--ink-faint); }
  footer { margin-top: 22px; font-size: 12px; color: var(--ink-faint); line-height: 1.7; }
</style>
<div class="wrap">
  <p class="eyebrow">Isan Household Financial Fragility Toolkit — อัปเดตอัตโนมัติรายเดือน</p>
...
<script>
  ...
  GROUPS.forEach(function (g) {
    ...
    var colorVar = CAT_COLOR_VAR[g.cat] || '--c-other';
    var color = getComputedStyle(document.documentElement).getPropertyValue(colorVar).trim();
    var section = document.createElement('div'); section.className = 'group';
    var head = document.createElement('div'); head.className = 'group-head';
    var dot = document.createElement('span'); dot.className = 'group-dot'; dot.style.background = color; head.appendChild(dot);
    var title = document.createElement('span'); title.className = 'group-title'; title.textContent = g.label; head.appendChild(title);
    var cnt = document.createElement('span'); cnt.className = 'group-count'; cnt.textContent = '(' + g.words.length + ' คำ)'; head.appendChild(cnt);
    section.appendChild(head);
    g.words.forEach(function (w) {
      var row = document.createElement('div'); row.className = 'row';
      var wordEl = document.createElement('div'); wordEl.className = 'row-word'; wordEl.textContent = w.text; row.appendChild(wordEl);
      var track = document.createElement('div'); track.className = 'row-track';
      var bar = document.createElement('div'); bar.className = 'row-bar'; bar.style.width = Math.max(4, (w.count / maxCount) * 100) + '%'; bar.style.background = color; track.appendChild(bar);
      row.appendChild(track);
      var countEl = document.createElement('div'); countEl.className = 'row-count'; countEl.textContent = w.count; row.appendChild(countEl);
