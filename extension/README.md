...
ผ่านง่ายกว่า pytrends มาก (เป็น browser จริง IP จริงของผู้ใช้) พิสูจน์มาแล้วจากโปรเจคต้นทาง (Isan Labor) กว่า 300 jobs
...
2. ตั้งที่เก็บดาวน์โหลด (`chrome://settings/downloads`) เป็นโฟลเดอร์ `incoming/` ของ repo นี้
   ใช้ `cd incoming && pwd` ที่ root repo เพื่อดู absolute path และปิด **Ask where to save each file**
