...
Examples:
  python -X utf8 collector/add_keyword.py "จำนำทะเบียนรถ" --prefix FP
...
# Same two-letter scheme as the original labor-market toolkit, reinterpreted
# for household financial fragility: first letter is the credit channel
# (Formal institution / Informal-Traditional / Informal-New digital), second
# is the direction — Pull = actively seeking credit, Push = distress signal
# (can't pay, being chased, losing an asset).
PREFIXES = {
    "FP": ("Formal", "Pull"),
    "FU": ("Formal", "Push"),
...
GROUP_MENU = [
    ("FP", "สถาบันการเงินในระบบ - แสวงหาสินเชื่อ", "สินเชื่อธนาคาร, บัตรกดเงินสด, กยศ"),
    ("FU", "สถาบันการเงินในระบบ - สัญญาณเปราะบาง", "พักหนี้, ประนอมหนี้ธนาคาร, หนี้เสีย NPL"),
    ("NP", "ช่องทางดิจิทัลใหม่ - แสวงหาสินเชื่อ", "กู้เงินด่วนออนไลน์, สินเชื่อออนไลน์"),
    ("NU", "ช่องทางดิจิทัลใหม่ - สัญญาณเปราะบาง", "หนี้แอปเงินกู้, โดนหลอกกู้เงินออนไลน์"),
    ("TP", "นอกระบบดั้งเดิม - แสวงหาสินเชื่อ", "จำนำทอง, ขายฝากที่ดิน, เงินกู้นอกระบบ"),
    ("TU", "นอกระบบดั้งเดิม - สัญญาณเปราะบาง", "หนี้นอกระบบ, ดอกเบี้ยโหด, ถูกยึดทรัพย์"),
]
...
def interactive_add(id_file: Path | None = None) -> int:
    ...
    The prompts live here rather than in scripts/toolkit.sh so all Thai UI text
    stays in one place regardless of which shell wraps the toolkit.
    """
...
def add(keyword: str, prefix: str, *, force: bool = False, id_file: Path | None = None) -> int:
    ...
    # Stable machine-readable line so toolkit.sh can pick the id up without
    # parsing Thai output.
...
def finalize(keyword_id: str, family_id: str | None) -> int:
    ...
    if suggested == "T1":
        ...
        print("ขั้นถัดไป: ./scripts/toolkit.sh monthly-finish")
        return 0
