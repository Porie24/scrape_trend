...
def discover_x13(root: Path, explicit: str | None = None) -> Path:
    ...
    raise PipelineError(
        "X-13 binary not found. Install it manually (see scripts/bootstrap-mac.sh) "
        "and place it at .tools/x13/1.1-b62/x13as, put it on PATH, or pass --x13-path. "
        "Non-Windows builds will not match EXPECTED_SHA256/EXPECTED_VERSION "
        "(those are pinned to the Windows 1.1-b62 release) — pass "
        "--allow-unverified-x13 once you've confirmed the binary yourself." + detail
    )
