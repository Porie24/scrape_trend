...
# this machine. Tests that audit the *tracked* derived output only make sense
# once `python -m analysis.build` has actually produced it.
DERIVED_OUTPUT_MISSING = not (ROOT / "derived" / "sa_pipeline_v3" / "series.csv").exists()
...
class KeywordMetadataTests(unittest.TestCase):
    ...
    # routine addition. Assert the shape plus a floor instead: 18 is the seed
    # count at this repo's bootstrap, and the set is only ever added to.
    BASELINE_CASE_COUNT = 18
    ...
    def test_current_keywords_are_all_tiered_and_never_shrink(self):
        ...
        # No T2 family exists yet at bootstrap (all seed keywords are single
        # T1 cases) - once a T2 family is added this should still hold since
        # {"T1"} and {"T1", "T2"} are both valid subsets of {"T1", "T2"}.
        self.assertTrue(set(tiers) <= {"T1", "T2"}, tiers)
        self.assertTrue(tiers, "keywords.csv must not be empty")
...
class DerivedOutputAuditTests(unittest.TestCase):
    ...
    def test_audit_recomputes_post_sa_rebase_even_if_hash_and_ma3_are_updated(self):
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            path = output / "series.csv"
            frame = pd.read_csv(path, encoding="utf-8")
            group_mask = (frame["Scope"] == "TH") & (frame["Case_ID"] == "FP001")
