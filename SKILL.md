---
name: isan-household-fragility-toolkit
description: Operate the Isan Household Financial Fragility Toolkit - collect canonical monthly CSVs through the Chrome extension, ingest and audit raw data, build the portable T1/T2 X-13/STL rebase and centered-MA3 analytical dataset, add/check keywords, and publish verified outputs. Trigger when the user asks to update data, run the extension, ingest CSVs, rebuild SA/T2/Rebase/MA3, add a keyword, check data health, move to another machine, or troubleshoot this repository.
---

# Isan Household Financial Fragility Toolkit - คู่มือปฏิบัติงานสำหรับ AI

คุณคือผู้ดูแลชุดข้อมูล Google Trends ของคำค้นความเปราะบางด้านการเงินครัวเรือนภาคอีสาน หน้าที่: อัพเดทข้อมูล ตรวจสุขภาพข้อมูล และเผยแพร่ อย่างปลอดภัยตามกติกาในไฟล์นี้ ถ้าคุณรันคำสั่งเองไม่ได้ ให้บอกคำสั่งทีละขั้นแล้วขอผลลัพธ์กลับมาตรวจ

repo นี้รันบน **macOS/Linux** — `scripts/toolkit.sh` เป็น bash port ของ `toolkit.ps1` ต้นฉบับ (ซึ่งเขียนไว้สำหรับ Windows) X-13 binary ต้องติดตั้งเองครั้งแรกเพราะไม่มี auto-installer สำหรับ Mac ดู README.md หัวข้อ "แพลตฟอร์มและ X-13"
...
| ขั้น | เจ้าของงาน |
|---|---|
| สร้างคิว | Agent รัน `./scripts/toolkit.sh monthly-prepare` |
| ดาวน์โหลด CSV รายเดือน | extension ใน Chrome profile ที่ลงชื่อเข้าใช้ Google แล้ว |
| ขั้นที่ผู้ใช้ต้องทำ | ครั้งแรกติดตั้ง extension/ตั้ง Downloads; แต่ละรอบ Import `jobs.json` + Start; แก้ CAPTCHA เมื่อพบ |
| ตรวจ/เข้าคลัง/publish | Agent รัน `./scripts/toolkit.sh monthly-finish`, ตรวจ stage allowlist แล้วจึง commit/push |

เมื่อผู้ใช้สั่ง "อัพเดทข้อมูล" ให้ใช้เส้นทางนี้ทันที ห้ามแจกแจงหลายเครื่องมือให้ผู้ใช้เลือก Python browser runner, pytrends เป็นทาง diagnostic สำหรับนักพัฒนาเท่านั้น ผลจากทางเหล่านั้น **ไม่ใช่ canonical data และห้าม ingest/stage/publish**

บนเครื่องใหม่ ให้ Agent รัน `./scripts/toolkit.sh setup` คำสั่งเดียว เพื่อเตรียม `.venv` และตรวจ environment/audits/tests ครบ (X-13 ต้องติดตั้งเองครั้งแรก — ไม่มี auto-installer บน Mac) ข้อมูลถาวรอยู่ใน GitHub แต่ extension, download path, GitHub auth, `incoming/`, jobs, queue state, `.venv/` และ `.tools/` เป็นของเฉพาะเครื่อง

## โครง repo

| ที่อยู่ | คืออะไร |
|---|---|
| `keywords.csv` | คำค้นที่ใช้งาน (เริ่มต้น 47 คำ seed, ID/คำ/Tier/Segment/Factor) แก้ไฟล์นี้เมื่อเพิ่ม/ถอดคำ |
...
| `collector/add_keyword.py` | เพิ่ม/ถอน/screen คำใหม่ตาม taxonomy (PREFIXES: FP/FU/TP/TU/NP/NU) |
...
| `scripts/toolkit.sh` | entrypoint ทางการ: `setup`, `monthly-run`, `monthly-prepare`, `monthly-finish` |
| `scripts/bootstrap-mac.sh` | คำสั่งระดับล่างที่ `toolkit.sh setup` เรียก เพื่อเตรียม `.venv` + pinned analysis dependencies (X-13 ต้องติดตั้งเอง) |
| `index.html` | หน้าแสดงผล (เปิด local ได้ หรือผ่าน GitHub Pages หลัง deploy) |
...
ID prefix 2 ตัวอักษร: ตัวแรกคือ Segment (ช่องทางสินเชื่อ), ตัวที่สองคือ Factor (ทิศทาง)

| Prefix | Segment | Factor |
|---|---|---|
| `FP` | Formal (สถาบันการเงินในระบบ) | Pull — แสวงหาสินเชื่อ |
| `FU` | Formal | Push — สัญญาณเปราะบาง |
| `TP` | Informal-Traditional (จำนำ/ขายฝาก/นอกระบบดั้งเดิม) | Pull |
| `TU` | Informal-Traditional | Push |
| `NP` | Informal-New (ช่องทางดิจิทัลใหม่) | Pull |
...
9. **`keywords.csv` เริ่มต้นเป็น seed ที่ยังไม่ผ่านด่านคัดกรองจริงจากข้อมูล** (ดูหมายเหตุใน README.md) หลังเก็บรอบแรก ให้รัน `check_keyword.py`/`audit.py` ทวนทุกคำแล้วตัดคำสัญญาณอ่อนออก อย่าสมมติว่าทุกคำใน seed จะผ่าน

## Workflow

### A. Monthly update production

ทางปกติให้รัน `./scripts/toolkit.sh monthly-run`; คำสั่งสร้างคิวแล้ว pause ที่ Chrome checkpoint และจะเรียก finish ต่อเมื่อผู้ใช้พิมพ์ `FINISH` หลัง Controller เหลือ 0 FAILED งานเท่านั้น

1. `./scripts/toolkit.sh monthly-prepare` สร้างคิวงานทั้งหมด (จำกัด scope ได้ เช่น `./scripts/toolkit.sh monthly-prepare --ids FP001 --geo TH`)
...
2. ให้ผู้ใช้ทำใน Chrome: คลิกไอคอน > Open Controller > **Import jobs.json** > เลือก `extension/data/jobs.json` > **Start**
   (ครั้งแรก: ติดตั้งแบบ Load unpacked + ตั้ง Downloads เป็น `incoming/` + ปิด Ask where to save ดู `extension/README.md`)
...
2. `./scripts/toolkit.sh monthly-finish` ตรวจการจับคู่ก่อน แล้วจึง ingest และรัน release gates ทั้งชุด
...
python collector/make_jobs.py --ids FP001 --geo TH
...
runner ใช้ persistent profile และเก็บไฟล์ที่ parser + canonical guard ตรวจแล้วไว้เฉพาะ `.browser-runner/captured/` โดยไม่เขียน `incoming/` ผลจาก runner ไม่ใช่ canonical และห้าม ingest/stage/publish

### C. ทดลอง pytrends (diagnostic เท่านั้น ห้าม publish)

```
pip install -r requirements.txt        (ครั้งแรกครั้งเดียว)
python collector/collect.py --plan --ids FP001,FU001     ดูงานก่อน
python collector/collect.py --ids FP001,FU001            ทดลองจริง (เกิด local changes)
```
scope อื่น: `--group FP,FU` / `--all` / `--geo TH` ส่วน `--start` และ `--end` มีไว้แสดง canonical window เท่านั้น ค่าอื่นจะถูก guard ปฏิเสธ ผลไม่ใช่ canonical และห้าม ingest/stage/publish

### D. เพิ่มคำใหม่

1. เช็ค `reference/keywords_tried.csv` ว่าเคยลองหรือยัง (กติกาเหล็กข้อ 6)
2. เพิ่มแถวใน `keywords.csv`: ตั้ง `Keyword_ID` ตาม pattern กลุ่ม (FP/FU/NP/NU/TP/TU + เลข 3 หลักที่ไม่ซ้ำทั้งใน keywords.csv และ keywords_tried.csv) หรือใช้ `python collector/add_keyword.py --interactive`
3. เก็บข้อมูลเฉพาะ ID นั้นด้วยเส้นทาง A (`./scripts/toolkit.sh monthly-prepare --ids <ID>`) เท่านั้น; pytrends/browser runner เป็น diagnostic และห้ามใช้เพิ่มหรืออัพเดท canonical data
4. ตรวจว่าโผล่ในหน้าเว็บแล้วค่อย push

### E. ตรวจก่อน push (ทำทุกครั้งที่ข้อมูลเปลี่ยน)

1. รัน `./scripts/toolkit.sh monthly-finish` (ล็อกเดือนได้ เช่น `monthly-finish 2026-06`) คำสั่งนี้ทำ ingest dry-run/จริง → raw structural/freshness gates → site check → analytical build/byte-check/audit → full tests → `git status` และหยุดเมื่อ native exit code ใดผิดปกติ โดยไม่ stage/commit/push
...
5. stage เฉพาะ allowlist แล้วตรวจรายชื่อก่อน commit:
   ...
   git push
   ```
   ถ้าแก้ `keywords.csv` ให้ stage แยกโดยตั้งใจ ห้ามใช้ `git add -A` แล้วรอ Pages rebuild ราว 1-2 นาที (หลังตั้ง Pages source เป็น GitHub Actions)

### F. อ่านผล data health ให้ถูก
...
- ด่านคัดกรองคำใหม่ใช้ตัวเลขชุดเดียวกัน: `python collector/check_keyword.py <ID>` บอก National / Regional / Tier ที่ควรเป็น (อ่านอย่างเดียว ต้องเก็บข้อมูลคำนั้นก่อน)
- `WEAK`, all-zero หรือ no-data คือข้อจำกัดของสัญญาณค้นหา **ไม่ใช่หลักฐานว่าครัวเรือนไม่มีความเปราะบางทางการเงิน**
...
| Python runner: `CHART_TIMEOUT`/หน้าให้ลงชื่อเข้าใช้ | Explore ใหม่ต้องใช้ authenticated profile; ใช้ extension ใน Chrome ปกติ ห้าม fallback กลับหน้า classic เพื่อ publish |
...
| `analysis.build` หา X-13 ไม่เจอ | ติดตั้ง binary เองตาม README.md หัวข้อ "แพลตฟอร์มและ X-13" แล้วใส่ `--x13-path` หรือตั้ง `$X13PATH`; ถ้า binary ไม่ใช่ Windows build ให้เติม `--allow-unverified-x13` |
